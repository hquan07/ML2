...
os.makedirs('visualizations/HeartDisease', exist_ok=True)
...
# Calculate Mean, Variance for numerical features
print("\nMean and Variance for Numerical Features:")
numerical_mean = heart_df_imputed[numeric_cols].mean()
numerical_variance = heart_df_imputed[numeric_cols].var()

mean_variance_df = pd.DataFrame({
    'Mean': numerical_mean,
    'Variance': numerical_variance
})
print(mean_variance_df)

# Calculate Covariance matrix for numerical features
numerical_covariance = heart_df_imputed[numeric_cols].cov()
print("\nCovariance Matrix for Numerical Features:")
print(numerical_covariance)

# Visualize Covariance Matrix
plt.figure(figsize=(10, 8))
sns.heatmap(numerical_covariance, annot=True, cmap='viridis')
plt.title('Covariance Matrix of Numerical Features')
plt.tight_layout()
plt.savefig('visualizations/HeartDisease/heart_covariance.png')
